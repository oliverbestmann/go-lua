...
func Benchmark_Fannkuch(b *testing.B) {
	l := NewStateEx()

	if err := DoString(l, luaSource); err != nil {
		b.Fatal("could not initialize benchmark", err)
	}

	b.ResetTimer()

	if err := DoString(l, fmt.Sprintf("run_iter(%d)", b.N)); err != nil {
		b.Fatal("error running benchmark", err)
	}
}
